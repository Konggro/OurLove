...
import { Heart, Sparkles, Plus, X, Trash2, Edit } from 'lucide-react';
...
export function ReasonsILoveYou({ onBack }: { onBack: () => void }) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen p-8 pt-24 flex items-center justify-center">
        <div className="text-white text-xl">Loading reasons...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen p-8 pt-24 flex flex-col items-center justify-center">
      ...
      {/* Manage Reasons List */}
      {reasons.length > 0 && (
        <div className="mb-6 w-full max-w-md max-h-40 overflow-y-auto">
      ...
      <AnimatePresence>
        {currentReasons.map((reason) => (
          <motion.div
            key={reason.id}
            className="fixed bottom-0 px-6 py-4 rounded-2xl bg-gradient-to-br from-[#C7AFFF]/90 to-[#FFC8E2]/90 backdrop-blur-md border border-white/20 shadow-2xl max-w-md"
            style={{
              left: `${reason.x}%`,
            }}
            initial={{ 
          ...
          >
            <p className="text-white text-center">{reason.text}</p>
      ...
      <AnimatePresence>
        {currentReasons.map((reason) => (
          [...Array(5)].map((_, i) => (
            ...
            >
              <Heart className="w-6 h-6 fill-[#FFC8E2] text-[#FFC8E2]" />
            </motion.div>
          ))
        ))}
      </AnimatePresence>
